...
export default class JAssemDetails extends React.Component{
    constructor(props){
        super(props);
    ...
    clickhandler(){
        ...
        axios.post("https://highgrip.in/api/JInsertassout",{sizes:this.state.inputData,id:this.state.id,to:this.state.slec})
        .then(response=>{
        ...
        if(isLoading){
            ...
            var _items = dataSource.map((key,value) => {
                return <View  key={value} style={styles.bg}>
                        ...
                        <TextInput style={{borderWidth:1,alignSelf:"flex-start",color:"#fff",padding:10,width:"20%", height:"75%", marginRight:10, color:"#000",textAlign:"center",}}  placeholder="En. Bag" onChangeText={(text) => this.addValues(text, key.id)} />
