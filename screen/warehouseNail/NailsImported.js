...
export default class NailsImported extends React.Component{
    constructor(props){
        super(props);
        this.state={
            data:[],
    ...
    componentDidMount(){
        return fetch('https://highgrip.in/api/Getimportedremain')
        .then((response)=>response.json())
        .then((responseJson)=>{
            const ds=[]
            responseJson.map((item,value)=>{
                ds.push({key:value,quantity:item.Quantity,size:item.size});
        ...
        if(this.state.isLoading){
            ...
            return(
                    ...
                    <FlatList data={this.state.dataSource}
                        ...
                        <Text style={styles.material}>{itmes.item.size}</Text>
