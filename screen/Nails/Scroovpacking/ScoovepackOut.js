...
export default class ScoovepackOut extends React.Component{
    constructor(props){
        super(props);
    ...
    componentDidMount(){
        return fetch("https://highgrip.in/api/scroovepackingsize")
      ...
      clickhandler(){
        ...
        if(this.state.inputData&&this.state.inputData.length){
            if(this.state.slec&&this.state.slec.length){
                ...
                axios.post("https://highgrip.in/api/ScroovepackingOut",{box:this.state.inputData,brand:this.state.slec})
                .then((response)=>{
        ...
        if(this.state.isLoading){
           return <View>
            ...
            let _itm=this.state.data.map(valu=>
                (
                <View key={valu.id} style={styles.bg}>
...
const styles = StyleSheet.create({
    ...
    insize:{borderWidth:1,alignSelf:"flex-start",color:"#fff",padding:10,width:"30%", marginRight:10, color:"#000",textAlign:"center",},
    inbag:{borderWidth:1,alignSelf:"flex-start",color:"#fff",padding:10,width:"30%", height:"75%", marginRight:10, color:"#000",textAlign:"center",},
