...
export default class FunraceScrooveStock extends React.Component{
    constructor(props){
        super(props);
        this.state={
            data:[],
    ...
    componentDidMount(){
        return fetch('https://highgrip.in/api/getremainfransScroove')
        .then((response)=>response.json())
        .then((responseJson)=>{
            const ds=[]
            responseJson.map((item,value)=>{
                ds.push({key:value,quantity:item.Quantity,code:item.code,thiknesh:item.thiknesh,length:item.length});
