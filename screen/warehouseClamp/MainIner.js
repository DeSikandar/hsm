...
export default class WareInner extends React.Component{
    constructor(props){
        super(props);
    ...
    componentDidMount(){
        ...
        .then((responseJson)=>{
            ...
            return fetch("https://highgrip.in/api/getSizeforclam")
            .then((resp)=>resp.json())
            .then((jrs)=>{
                this.setState({
                    sizes:jrs,
    ...
    if(this.state.inputData && this.state.inputData.length){
        if(this.state.slec&& this.state.slec.length){
            ...
            if(this.state.slec.length == this.state.inputData.length){
                ...
                axios.post("https://highgrip.in/api/InsertTestIn",{pid:1,brands:this.state.slec,weight:this.state.inputData})
                .then(response=>{
        ...
        if(this.state.isLoading){
            ...
            let _itms=this.state.sizes.map(vlue=>(
                ...
                <View key={vlue.id}   style={styles.container}>
                    ...
                    <TextInput style={{borderWidth:1,alignSelf:"flex-start",padding:10,width:"22%",height:"78%",}} placeholder="Weight" onChangeText={(valu)=>this.addValues(valu,vlue.id)} />
                </View>
...
const styles = StyleSheet.create({
    ...
    weight:{
         padding:7,width:"22%", fontFamily:"serif",textShadowColor:"#000", textShadowColor: 'black',  textShadowOffset: { width: 1, height: 1 },
